...
int main(int argc, char **argv)
{
    ...
    rc = sh1106_init(i2c_node_address);
    ...
    if (oled_type[0] != 0)
    {
        if (strcmp(oled_type, "128x64") == 0)
            rc += sh1106_oled_default_config(64, 128);
        else if (strcmp(oled_type, "128x32") == 0)
            rc += sh1106_oled_default_config(32, 128);
        else if (strcmp(oled_type, "64x48") == 0)
            rc += sh1106_oled_default_config(48, 64);
    }
    else if (sh1106_oled_load_resolution() != 0)
    {
    ...
    if (clear_all > -1)
    {
        rc += sh1106_oled_clear_screen();
    }
    else if (clear_line > -1)
    {
        rc += sh1106_oled_clear_line(clear_line);
    }
    
    // set rotate orientation
    if (orientation > -1)
    {
        rc += sh1106_oled_set_rotate(orientation);
    }
    
    // set oled inverted
    if (inverted > -1)
    {
        rc += sh1106_oled_display_flip(inverted);
    }
    
    // set display on off
    if (display > -1)
    {
        rc += sh1106_oled_onoff(display);
    }
    
    // set cursor XY
    if (x > -1 && y > -1)
    {
        rc += sh1106_oled_set_XY(x, y);
    }
    else if (x > -1)
    {
        rc += sh1106_oled_set_X(x);
    }
    else if (y > -1)
    {
        rc += sh1106_oled_set_Y(y);
    }
    
    // print text
    if (msg[0] != 0)
    {
        rc += sh1106_oled_write_string(font, msg);
    }
    else if (line[0] != 0)
    {
        rc += sh1106_oled_write_line(font, line);
    }    
