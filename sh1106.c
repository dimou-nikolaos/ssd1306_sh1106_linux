...
const char init_oled_type_file[] = "/tmp/.sh1106_oled_type";
...
{
    uint8_t rc;
    rc = _i2c_init(i2c_dev, SH1106_I2C_ADDR);
    if (rc > 0)
        return rc;
        
    // test i2c connection
    uint8_t cmd = SH1106_COMM_CONTROL_BYTE;
    uint8_t result = 0;
    _i2c_write(&cmd, 1);
    _i2c_read(&result, 1);
    if (result == 0)
...
{
    data_buf[0] = SH1106_COMM_CONTROL_BYTE;
    if (onoff == 0)
        data_buf[1] = SH1106_COMM_DISPLAY_OFF;
    else
        data_buf[1] = SH1106_COMM_DISPLAY_ON;
...
uint8_t sh1106_oled_horizontal_flip(uint8_t flip)
{
    data_buf[0] = SH1106_COMM_CONTROL_BYTE;
    if (flip == 0)
        data_buf[1] = SH1106_COMM_HORIZ_NORM;
    else
        data_buf[1] = SH1106_COMM_HORIZ_FLIP;
...
uint8_t sh1106_oled_display_flip(uint8_t flip)
{
    data_buf[0] = SH1106_COMM_CONTROL_BYTE;
    if (flip == 0)
        data_buf[1] = SH1106_COMM_DISP_NORM;
    else
        data_buf[1] = SH1106_COMM_DISP_INVERSE;
...
{
    data_buf[0] = SH1106_COMM_CONTROL_BYTE;
    data_buf[1] = SH1106_COMM_MULTIPLEX;
    data_buf[2] = row - 1;
...
{
    data_buf[0] = SH1106_COMM_CONTROL_BYTE;
    data_buf[1] = SH1106_COMM_VERT_OFFSET;
    data_buf[2] = offset;
...
{
    data_buf[0] = SH1106_COMM_CONTROL_BYTE;
    data_buf[1] = SH1106_COMM_CLK_SET;
    data_buf[2] = clk;
...
{
    data_buf[0] = SH1106_COMM_CONTROL_BYTE;
    data_buf[1] = SH1106_COMM_PRECHARGE;
...
{
    data_buf[0] = SH1106_COMM_CONTROL_BYTE;
    data_buf[1] = SH1106_COMM_DESELECT_LV;
    data_buf[2] = voltage;
...
{
    data_buf[0] = SH1106_COMM_CONTROL_BYTE;
    data_buf[1] = SH1106_COMM_COM_PIN;
    data_buf[2] = value;
    
    return _i2c_write(data_buf, 3);
}

// default value use SH1106_PAGE_MODE
uint8_t sh1106_oled_set_mem_mode(uint8_t mode)
{
    data_buf[0] = SH1106_COMM_CONTROL_BYTE;
    data_buf[1] = SH1106_COMM_MEMORY_MODE;
    data_buf[2] = mode;
...
{
    data_buf[0] = SH1106_COMM_CONTROL_BYTE;
    data_buf[1] = SH1106_COMM_SET_COL_ADDR;
    data_buf[2] = start;
    data_buf[3] = end;
...
{
    data_buf[0] = SH1106_COMM_CONTROL_BYTE;
    data_buf[1] = SH1106_COMM_SET_PAGE_ADDR;
    data_buf[2] = start;
    data_buf[3] = end;
...
{
    data_buf[0] = SH1106_COMM_CONTROL_BYTE;
    data_buf[1] = SH1106_COMM_CONTRAST;
    data_buf[2] = value;
...
uint8_t sh1106_oled_scroll_onoff(uint8_t onoff)
{
    data_buf[0] = SH1106_COMM_CONTROL_BYTE;
    if (onoff == 0)
        data_buf[1] = SH1106_COMM_DISABLE_SCROLL;
    else
        data_buf[1] = SH1106_COMM_ENABLE_SCROLL;
...
uint8_t sh1106_oled_set_X(uint8_t x)
{
    ...
    // SH1106 needs column offset
    x += SH1106_COLUMN_OFFSET;
    
    data_buf[0] = SH1106_COMM_CONTROL_BYTE;
    data_buf[1] = SH1106_COMM_LOW_COLUMN | (x & 0x0f);
    data_buf[2] = SH1106_COMM_HIGH_COLUMN | ((x >> 4) & 0x0f);
...
uint8_t sh1106_oled_set_Y(uint8_t y)
{
    ...
    data_buf[0] = SH1106_COMM_CONTROL_BYTE;
    data_buf[1] = SH1106_COMM_PAGE_NUMBER | (y & 0x0f);
...
uint8_t sh1106_oled_set_XY(uint8_t x, uint8_t y)
{
    ...
    data_buf[0] = SH1106_COMM_CONTROL_BYTE;
    data_buf[1] = SH1106_COMM_PAGE_NUMBER | (y & 0x0f);
    data_buf[2] = SH1106_COMM_LOW_COLUMN | (x & 0x0f);
    data_buf[3] = SH1106_COMM_HIGH_COLUMN | ((x >> 4) & 0x0f);
...
uint8_t sh1106_oled_set_rotate(uint8_t degree)
{
    ...
    if (degree == 0)
    {
        data_buf[0] = SH1106_COMM_CONTROL_BYTE;
        data_buf[1] = SH1106_COMM_HORIZ_FLIP;
        data_buf[2] = SH1106_COMM_SCAN_REVS;
    ...
    else if (degree == 180)
    {
        data_buf[0] = SH1106_COMM_CONTROL_BYTE;
        data_buf[1] = SH1106_COMM_HORIZ_NORM;
        data_buf[2] = SH1106_COMM_SCAN_NORM;
...
{
    if (oled_lines != SH1106_128_64_LINES && oled_lines != SH1106_128_32_LINES && SH1106_64_48_LINES)
        oled_lines = SH1106_128_64_LINES;
        
    if (oled_columns != SH1106_128_64_COLUMNS && oled_lines != SH1106_128_32_COLUMNS && SH1106_64_48_COLUMNS)
        oled_columns = SH1106_128_64_COLUMNS;
    ...
    if (sh1106_oled_save_resolution(max_columns, max_lines) != 0)
        return 1;
    
    uint16_t i = 0;
    data_buf[i++] = SH1106_COMM_CONTROL_BYTE;  //command control byte
    data_buf[i++] = SH1106_COMM_DISPLAY_OFF;   //display off
    data_buf[i++] = SH1106_COMM_DISP_NORM;     //Set Normal Display (default)
    data_buf[i++] = SH1106_COMM_CLK_SET;       //SETDISPLAYCLOCKDIV
    data_buf[i++] = 0x80;                      // the suggested ratio 0x80
    data_buf[i++] = SH1106_COMM_MULTIPLEX;     //SH1106_SETMULTIPLEX
    data_buf[i++] = oled_lines - 1;            // height is 32 or 64 (always -1)
    data_buf[i++] = SH1106_COMM_VERT_OFFSET;   //SETDISPLAYOFFSET
    data_buf[i++] = 0;                         //no offset
    data_buf[i++] = SH1106_COMM_START_LINE;    //SETSTARTLINE
    data_buf[i++] = SH1106_COMM_CHARGE_PUMP;   //CHARGEPUMP
    data_buf[i++] = 0x14;                      //turn on charge pump
    // Note: SH1106 doesn't use memory mode commands
    data_buf[i++] = SH1106_COMM_HORIZ_NORM;    //SEGREMAP  Mirror screen horizontally (A0)
    data_buf[i++] = SH1106_COMM_SCAN_NORM;     //COMSCANDEC Rotate screen vertically (C0)
    data_buf[i++] = SH1106_COMM_COM_PIN;       //HARDWARE PIN 
    if (oled_lines == 32)
        data_buf[i++] = 0x02;                  // for 32 lines
    else
        data_buf[i++] = 0x12;                  // for 64 lines or 48 lines
    data_buf[i++] = SH1106_COMM_CONTRAST;      //SETCONTRAST
    data_buf[i++] = 0x7f;                      // default contract value
    data_buf[i++] = SH1106_COMM_PRECHARGE;     //SETPRECHARGE
    data_buf[i++] = 0xf1;                      // default precharge value
    data_buf[i++] = SH1106_COMM_DESELECT_LV;   //SETVCOMDETECT                
    data_buf[i++] = 0x40;                      // default deselect value
    data_buf[i++] = SH1106_COMM_RESUME_RAM;    //DISPLAYALLON_RESUME
    data_buf[i++] = SH1106_COMM_DISP_NORM;     //NORMALDISPLAY
    data_buf[i++] = SH1106_COMM_DISPLAY_ON;    //DISPLAY ON             
    data_buf[i++] = SH1106_COMM_DISABLE_SCROLL;//Stop scroll
...
uint8_t sh1106_oled_write_line(uint8_t size, char* ptr)
{
    ...
    if (size == SH1106_FONT_SMALL) // 5x7
    {
        font_table = (uint8_t*)font5x7;
        font_table_width = 5;
    }
    else if (size == SH1106_FONT_NORMAL) // 8x8
    {
    ...
    data_buf[i++] = SH1106_DATA_CONTROL_BYTE;
...
uint8_t sh1106_oled_write_string(uint8_t size, char* ptr)
{
    ...
    do {
        ...
        // set cursor position
        sh1106_oled_set_XY(global_x, global_y);
        rc += sh1106_oled_write_line(size, buf);
...
uint8_t sh1106_oled_clear_line(uint8_t row)
{
    ...
    sh1106_oled_set_XY(0, row);
    data_buf[0] = SH1106_DATA_CONTROL_BYTE;
